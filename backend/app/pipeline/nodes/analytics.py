...
def _py_float(value) -> float:
    return float(value)


def _py_int(value) -> int:
    return int(value)
...
def analytics_node(state: PipelineState) -> PipelineState:
    ...
    try:
        ...
        years = sorted(_py_int(y) for y in df["year"].unique().tolist())
        yearly_stats = []

        for year in years:
            ydf = df[df["year"] == year]
            premium = float(premium_by_year.get(year, Decimal("0")))
            total_incurred = _py_float(ydf["incurred_f"].sum())
            claim_count = len(ydf)
            ...
            yearly_stats.append({
                ...
                "total_incurred": round(_py_float(total_incurred), 2),
                "total_paid": round(_py_float(ydf["paid_f"].sum()), 2),
                "total_reserved": round(_py_float(ydf["reserved_f"].sum()), 2),
                ...
                "large_loss_count": _py_int(ydf["is_large"].sum()),
                "open_claim_count": _py_int(ydf["is_open"].sum()),
            })

        # Overall metrics
        total_incurred_all = _py_float(df["incurred_f"].sum())
        ...
        total_open_reserves = _py_float(df[df["is_open"]]["reserved_f"].sum())
        total_large_loss = _py_float(df[df["is_large"]]["incurred_f"].sum())
        ...
        logger.exception("Analytics node failed for job %s", job_id)
        db = SessionLocal()
        try:
            job = db.query(Job).filter(Job.id == job_id).first()
            if job:
                job.status = JobStatus.failed
                job.current_stage = "analytics_failed"
        ...
        return {
            **state,
            "errors": errors,
            "current_stage": "analytics_failed",
